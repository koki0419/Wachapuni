...
public class Slime_Children_B : Slime_ChildrenController
{
    ...
    {
        // プレイヤーのポジションを取得
        Vector3 playerpos = new Vector3(PlayerObj.transform.localPosition.x,        // 都度プレイヤー位置更新
                                        PlayerObj.transform.localPosition.y,        // 自分にプレイヤーポジションを入れる(ワープしてしまう)
                                        PlayerObj.transform.localPosition.z - 1.0f);// gameObject.transform.localPosition = playerpos;
        ...
        {
            gameObject.transform.localPosition += new Vector3(moveVec.x, moveVec.y, moveVec.z);
        }
        else if (Mathf.Abs(playerpos.z - gameObject.transform.localPosition.z) <= 0.5)
        {
            gameObject.transform.localPosition += new Vector3(moveVec.x, moveVec.y, moveVec.z - 0.1f);
        }
        else
        {
